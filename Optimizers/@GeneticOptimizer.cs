	...
	public class GeneticOptimizer : Optimizer
	{
		...
		public class Individual : ICloneable
		{
			...
			public override string ToString()
			{
				...
				{
					parameters.Append(i == 0 ? "[" : i < Parameters.Length ? ";" : string.Empty);
					parameters.Append(Parameters[i]);
					if (i == Parameters.Length - 1)
						parameters.Append("]");
				}

				return string.Format("{0} Type={1} Weight={2} Parameters={3} PerformanceValue={4}", IndividualName, Type, Weight, parameters.ToString(), PerformanceValue);
			}
		}
