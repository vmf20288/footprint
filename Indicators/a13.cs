    ...
    public class a13 : Indicator
    {
        ...
        protected override void OnRender(ChartControl chartControl, ChartScale chartScale)
        {
            ...
            using (var deltaFormat = new TextFormat(Core.Globals.DirectWriteFactory, "Arial", deltaFontSize))
            {
                ...
                float labelX = xFirst - 50f;

                DrawRowLabels(deltaFormat, labelX, yTop);
        ...
        private void DrawRowLabels(TextFormat fmt, float xLeft, float yTop)
        {
            const float labelWidth = 50f;
            string deltaLabel = "Delta";
            using (var layout = new TextLayout(Core.Globals.DirectWriteFactory, deltaLabel, fmt, labelWidth, rectHeight))
            {
                var m = layout.Metrics;
                float tx = xLeft + (labelWidth - m.Width) / 2f;
                float ty = yTop + (rectHeight - m.Height) / 2f;
                RenderTarget.DrawTextLayout(new Vector2(tx, ty), layout, brushGeneral);
            }

            string volLabel = "Volumen";
            using (var layout = new TextLayout(Core.Globals.DirectWriteFactory, volLabel, fmt, labelWidth, rectHeight))
            {
                var m = layout.Metrics;
                float tx = xLeft + (labelWidth - m.Width) / 2f;
                float ty = yTop + rectHeight + (rectHeight - m.Height) / 2f;
                RenderTarget.DrawTextLayout(new Vector2(tx, ty), layout, brushGeneral);
            }
        }
...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    {
        private a13[] cachea13;
        public a13 a13(int fontSizeProp)
        {
            return a13(Input, fontSizeProp);
        }

        public a13 a13(ISeries<double> input, int fontSizeProp)
        {
            if (cachea13 != null)
                for (int idx = 0; idx < cachea13.Length; idx++)
                    if (cachea13[idx] != null && cachea13[idx].FontSizeProp == fontSizeProp && cachea13[idx].EqualsInput(input))
                        return cachea13[idx];
            return CacheIndicator<a13>(new a13(){ FontSizeProp = fontSizeProp }, input, ref cachea13);
        }
    }
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
    ...
    {
        public Indicators.a13 a13(int fontSizeProp)
        {
            return indicator.a13(Input, fontSizeProp);
        }

        public Indicators.a13 a13(ISeries<double> input , int fontSizeProp)
        {
            return indicator.a13(input, fontSizeProp);
        }
    }
